...
def operation(db,unique_code,form_code):
    ...
    try:
        ...
        if results2[0]['university name']:
            #print results2[0]['institute name']
            #print form_code+'%'
            sql3 ="SELECT ROUND((SUM(`total`) / COUNT(`total`)),2) AS `AVG` FROM `undergraduate_record` INNER JOIN `undergraduate`ON `undergraduate_record`.`user_code` =`undergraduate`.`unique_code` AND `university name`= '%s' AND `form_code` LIKE '%s' GROUP BY `undergraduate_record`.`user_code`"%(results2[0]['university name'], form_code+'%')
